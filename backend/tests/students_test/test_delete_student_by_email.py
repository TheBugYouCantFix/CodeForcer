...
def test_does_not_delete_student_by_email_if_email_is_invalid(invalid_email, students_repo_mock):
    response = client.delete(f"/students/{invalid_email}")

    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


def test_does_not_delete_student_by_email_if_email_is_valid_but_does_not_exist(email, students_repo_mock):
